...
const StyledChip = styled(Chip)<StyledChipProps>(
  ({ theme, isselected, backgroundColor }) => ({
    marginRight: '16px',
    marginBottom: '16px',
    minWidth: '95px',
    minHeight: '50px',
...
const CreateRecordPage = (): ReactElement => {
  ...
  const [recoilSubCategoryValue, setRecoilSubCategoryValue] =
    useRecoilState(recoilSubCategory);
  ...
  function formatDate(date: Date): string {
    ...
    return `${year}-${String(month).padStart(2, '0')}-${String(day).padStart(
      ...
      2,
      '0',
    )}:${String(second).padStart(2, '0')} KST`;
  }
  ...
  > {
    const newRecord = {
      categoryId:
        categories[selectedCategoryIdx].subCategories[selectedSubCategoryIdx]
          .categoryId,
      name,
      startedAt,
      finishedAt,
    ...
    if (response.result) {
      const { categoryId, name, startedAt, finishedAt, timeUnit } =
        response.result;
      setSelectedDays([]);
      navigate('/record');
      return {
  ...
  async function postAllDays(
    ...
    for (const dayIndex of selectedDays) {
      const startedAtOfDay = formatDate(
        new Date(addDays(selectedDate.start, dayIndex - selectedDays[0])),
      );
      const finishedAtOfDay = formatDate(
        new Date(addDays(selectedDate.end, dayIndex - selectedDays[0])),
      );
      ...
      <Container>
        ...
        >
          <DaysChip
            title="일"
            isselected={
              selectedDate.start.getDay() === 0 || selectedDays.includes(0)
            }
            onClick={() => handleDayChipClick(0)}
            underline={new Date().getDay() === 0}
          />
          <DaysChip
            title="월"
            isselected={
              selectedDate.start.getDay() === 1 || selectedDays.includes(1)
            }
            onClick={() => handleDayChipClick(1)}
            underline={new Date().getDay() === 1}
          />
          <DaysChip
            title="화"
            isselected={
              selectedDate.start.getDay() === 2 || selectedDays.includes(2)
            }
            onClick={() => handleDayChipClick(2)}
            underline={new Date().getDay() === 2}
          />
          <DaysChip
            title="수"
            isselected={
              selectedDate.start.getDay() === 3 || selectedDays.includes(3)
            }
            onClick={() => handleDayChipClick(3)}
            underline={new Date().getDay() === 3}
          />
          <DaysChip
            title="목"
            isselected={
              selectedDate.start.getDay() === 4 || selectedDays.includes(4)
            }
            onClick={() => handleDayChipClick(4)}
            underline={new Date().getDay() === 4}
          />
          <DaysChip
            title="금"
            isselected={
              selectedDate.start.getDay() === 5 || selectedDays.includes(5)
            }
            onClick={() => handleDayChipClick(5)}
            underline={new Date().getDay() === 5}
          />
          <DaysChip
            title="토"
            isselected={
              selectedDate.start.getDay() === 6 || selectedDays.includes(6)
            }
            onClick={() => handleDayChipClick(6)}
            underline={new Date().getDay() === 6}
          />
        </Container>
        <Container
          sx={{
