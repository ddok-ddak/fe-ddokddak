...
const VerifyCode = (props: any) => {
  ...
  useEffect(() => {
    setSignUpStepInstruction('인증코드를 입력해주세요.');
    setSignUpNextButtonProps({
      text: '인증 완료',
      clickHandler: () => {
        setCurrentStepIndex(currentStepIndex + 1);
        if (verificationAttemptCount >= 5) {
          setModalInfo({ 
            open: true, 
            title: '인증코드 입력 횟수(5회)를 초과했습니다. 다른 이메일로 시도하시겠습니까? ', 
            msg: '그렇지 않은 경우 해당 이메일로는 24시간 후에 시도할 수 있습니다.',
            btn1Text: '아니오',
            btn1ClickHandler: () => {
              setModalInfo({ ...modalInfo, open: false });
    ...
    <FormWrapper>
      ...
      <Box sx={{position: 'relative', top: '15%'}}>
        <Typography
          ...
          >
        ! 인증 코드가 오지 않으면 스팸함을 확인해주세요. <br/>스팸함에도 인증 코드 메일이 오지 않았다면 dodone@gmail.com으로 문의를 남겨주세요. 감사합니다.
        </Typography>
      </Box>
    </FormWrapper>
  );
